...
if ( $config['environment'] == 'local' ) {
    define( 'DOMAIN', ( !empty( $_SERVER['HTTPS'] )? 'https' : 'http' ) . '://' . $_SERVER['HTTP_HOST'] . '/' . $config['appdirectory'] . '/' );
}
else {
    define( 'DOMAIN', ( !empty( $_SERVER['HTTPS'] )? 'https' : 'http' ) . '://' . $_SERVER['HTTP_HOST'] . '/' );
}

define( 'THEME', HTTP_SERVER . basename( dirname( __FILE__ ) ) . '/_public/_front/' . $config['template'] . '/' );
